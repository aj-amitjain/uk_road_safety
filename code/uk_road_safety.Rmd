...
-For female fatalties is high for yonger female which slight decrease for age group till 60, and then its again high for age 60 to aroud 90. 
...
Severity 2 (serious) have similar monday to friday increase trend. friday with higest count. 
...
ggplot(data=casualties_quarter[casualties_quarter$Casualty_Severity == 1, ], aes(x=Quarter, y=Count)) + 
        geom_point() + 
        geom_line() + 
        facet_wrap(~Year)
...
Over the year, There is an increasing trend for the fatalities case for the 1 to 4 quarter. 
For first quarter of year the count for fatalities have decreased but this is opposite for last 2 quarters.
...
There is a increasing trend from around 7am to 7pm over the year. 
